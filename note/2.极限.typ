...
#definition[
	函数在单点处的极限 $display(lim_(x->x_0^+) f(x) = A)$ 的定义：如果对于任意 $eps>0$，存在 $delta>0$ 使得 $forall x in (x_0,x_0 +delta)$，$|f(x)-A| < eps$，那么称 $f(x)$ 当 $x$ 从右侧趋于 $x_0$ 时收敛于 $A$，也记作 $f(x_0^+)$
...
#warning[使用时，需要注意取到极限的位置，实际上有 $display(lim_(x->oo) (sin x) / x = 0 != 1)$。]
