...
#def[定理1]*（极值的第一充分条件）*设 $f(x)$ 在点 $x_0$ 的邻域内连续，且在空心邻域内有导数，则

(1) 若 $f'(x)$ 在左邻域为正，右邻域为负，则 $f(x)$ 在点 $x_0$ 取极大值；\
(2) 若 $f'(x)$ 在左邻域为负，右邻域为正，则 $f(x)$ 在点 $x_0$ 取极小值；\
(3) 否则，则说明 $f(x_0)$ 不是极值．

#warn[空心领域内导函数存在，则两侧导函数 *必须* 异号．]

#def[定理2]*（极值的第二充分条件）*设 $f(x)$ 在点 $f(x_0)$ 处具有二阶导数且 $f'(x_0),sp f''(x_0) != 0$．若 $f''(x_0)<0$（$f''(x_0)>0$）则 $f(x)$ 在点 $x_0$ 处取极大值（极小值）．进一步的，若 $f(x)$ 在点 $x_0$ 有直到 $n$ 阶导数，且 $f'(x_0)=f''(x_0)=dots.c=f^((n-1))(x_0) = 0$，$f^((n))(x_0)!=0$．当 $n$ 为 *偶数* 时，$x_0$ 是极值点；否则，$x_0$ 不是极值点．
...
#def[拐点的必要条件]设函数 $f(x)$ 在 $x_0$ 的某邻域内具有二阶导数，若 $x_0$ 是 $y=f(x)$ 的拐点，则必有 $f''(x_0) = 0$．*反之不成立*．

#def[拐点的充分条件]设函数 $f(x)$ 在 $x_0$ 的某邻域内具有二阶导数，若在 $x_0$ 的两侧 $f''(x_0)$ 一号，则 $x_0$ 是 $y=f(x)$ 的拐点；反之，$x_0$ 就不是曲线的拐点．
...
当 $a=0$ 时，该渐近线又可称为水平渐近线，水平渐近线是斜渐近线的一种．
...
$x=x_0$ 是曲线 $y=f(x)$ 的垂直渐近线的充要条件是 $display(lim_(x->x_0) f(x) = oo)$（可以两侧分别是 $+oo$ 和 $-oo$）．
