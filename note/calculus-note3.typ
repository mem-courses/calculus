...
设 $y=f(x) sp (x in D), sp x_0 in D$，若 $exists delta > 0$，当 $0<|x-x_0|<delta$ 时，有 $f(x)<f(x_0)$ 则称 $x=x_0$ 为 $f(x)$ 的极大值点，$f(x_0)$ 为极大值。

设 $y=f(x) sp (x in D), sp x_0 in D$，若 $exists delta > 0$，当 $0<|x-x_0|<delta$ 时，有 $f(x)>f(x_0)$ 则称 $x=x_0$ 为 $f(x)$ 的极小值点，$f(x_0)$ 为极小值。
...
2. 若 $f(x)$ 在 $x=x_0$ 的去心领域内导函数存在且不为零，则两侧导函数必须异号。

=== 极值点的充分条件

（极值的第一充分条件）设 $f(x)$ 在点 $x_0$ 的邻域内连续，且在空心邻域内有导数，则
...
#note[
  这说明 $f(x)$ 的导数满足介值定理，但这并不意味着导函数连续，如 $f(x) = x^2 sin(display(1/x^2))$，但考虑
...
#def[曲线凹凸性的判定定理]设函数 $f(x)$ 在区间 $(a,b)$ 内具有二阶导数，那么：

#deft[曲线凹凸性的判定定理]1. 若 $x in (a,b)$ 有 $f''(x)>0$，则曲线 $y=f(x)$ 在 $(a,b)$ 内是凹的；
...
设函数 $f(x)$ 在 $x_0$ 的某邻域内具有二阶导数，若 $x_0$ 是 $y=f(x)$ 的拐点，则必有 $f''(x_0) = 0$．*反之不成立*．

=== 拐点的充分条件

1. 设函数 $f(x)$ 在 $x_0$ 的某邻域内具有二阶导数，若在 $x_0$ 的两侧 $f''(x_0)$ 异号，则 $(x_0,f(x_0))$ 是 $y=f(x)$ 的拐点；反之，$(x_0,f(x_0))$ 就不是曲线的拐点．

2. 设函数 $f(x)$ 在 $x_0$ 的某邻域内具有三阶导数，且满足 $f''(x_0)=0$，$f'''(x_0) != 0$，则 $(x_0,f(x_0))$ 是 $y=f(x)$ 的拐点。（如果 $f'''(x_0)=0$，也可以将结论进一步推广）
...
$x=x_0$ 是曲线 $y=f(x)$ 的垂直渐近线的充分必要条件是 $display(lim_(x->x_0) f(x) = oo)$（可以两侧分别是 $+oo$ 和 $-oo$）．
